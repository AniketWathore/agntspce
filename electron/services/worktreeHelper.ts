...
export class WorktreeHelper {
  ...
  async resolvePreferredBaseBranch(masterPath: string, preferredBranch = 'master'): Promise<string> {
    try {
      const head = await this.execGit('git symbolic-ref --short HEAD', masterPath)
      return head
    } catch { }

    const candidates = preferredBranch === 'master' ? ['main', 'master'] : [preferredBranch, 'main', 'master']
  ...
  async createWorktree(workspace: Workspace, worktreeId: string): Promise<string> {
    ...
    try {
      defaultBranch = await this.execGit('git symbolic-ref --short HEAD', masterPath)
    } catch {
      defaultBranch = repo.masterBranch || 'main'
      try {
        await this.execGit(`git rev-parse --verify ${defaultBranch}`, masterPath)
      } catch {
        const fallback = defaultBranch === 'main' ? 'master' : 'main'
        try {
          await this.execGit(`git rev-parse --verify ${fallback}`, masterPath)
          defaultBranch = fallback
        } catch {
          throw new Error(`No usable base branch found — tried ${defaultBranch}, ${fallback}`)
        }
      }
    }
