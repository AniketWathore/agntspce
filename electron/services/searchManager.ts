...
function getInstalledBinaryPath(): string | null {
  return findInstalledBinary(getInstalledBinaryCandidates())
}
...
function getSitePackagesDir(searchDir: string): string {
  const winDir = path.join(searchDir, 'python', 'Lib', 'site-packages')
...
function isPackageBroken(searchDir: string): boolean {
  const sitePkgs = getSitePackagesDir(searchDir)
  if (fs.existsSync(path.join(sitePkgs, 'semble'))) return false
...
function installSearch(): string | null {
  ...
  if (!bundled) {
    ...
    if (process.platform === 'win32') {
      console.log('[agntspce] Search not available on Windows (no prebuilt binary)')
    } else {
...
function injectOpenCodeConfig(): InjectResult {
  ...
  const entryValue = {
    command: process.platform === 'win32' ? ['cmd.exe', '/c', binaryPath] : [binaryPath],
    type: 'local',
    enabled: true,
    timeout: 120_000,
  }
  ...
  try {
    ...
    // Ensure watcher ignore pattern is set to prevent ConPTY path corruption on Windows
    if (!config.watcher || !config.watcher.ignore || !Array.isArray(config.watcher.ignore) || config.watcher.ignore.length === 0) {
    ...
    return existing === newValue ? { agent: 'opencode', action: 'unchanged' } : { agent: 'opencode', action: 'updated' }
  } catch (e) {
...
function injectOpenCodeConfigTextFallback(configPath: string, binaryPath: string): InjectResult {
  const entryText = `"agntspce-search": ${JSON.stringify({ command: process.platform === 'win32' ? ['cmd.exe', '/c', binaryPath] : [binaryPath], type: 'local', enabled: true, timeout: 120_000 }, null, 2)}`
