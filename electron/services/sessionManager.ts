...
function resolveValidCwd(candidate: string | undefined | null, ...fallbacks: (string | undefined | null)[]): string {
...
for (const dir of [
  ...
  // Dev mode fallback: process.cwd() is project root
  path.join(process.cwd(), 'bin'),
  // Dev mode fallback: dist-electron/rtk/ (post-build copies here)
  path.join(__dirname, 'rtk'),
]) {
...
if (AGNTSPCE_BIN_DIR) {
  console.log('[sessionManager] AGNTSPCE_BIN_DIR =', AGNTSPCE_BIN_DIR)
} else {
  console.warn('[sessionManager] AGNTSPCE_BIN_DIR not found — wrappers unavailable')
}
...
export class SessionManager extends EventEmitter {
  ...
  createSession(sessionId: string, config: SessionConfig) {
    ...
    // Log the PTY env setup for debugging Windows PATH issues
    if (process.platform === 'win32') {
      const diagPathKey = Object.keys(env).find(k => /^path$/i.test(k)) || 'PATH'
      console.log(`[sessionManager][win32] PTY env PATH key="${diagPathKey}" first 200="${(env[diagPathKey] || '').slice(0, 200)}"`)
    }
    ...
    let ptyProcess
    try {
      const spawnCwd = resolveValidCwd(config.cwd, os.homedir())
  ...
  createRawSession(type: string, workspacePath?: string, existingSessionId?: string): { sessionId: string } | null {
    ...
    const rawCwd = workspacePath || this.workspace?.repository?.path || process.env.HOME || os.homedir()
    const cwd = resolveValidCwd(rawCwd)
  ...
  async restoreSessions(sessions: SavedSessionData[]): Promise<void> {
    ...
    for (const saved of sessions) {
      ...
      const rawCwd = saved.cwd || this.workspace?.repository?.path || process.env.HOME
      const cwd = resolveValidCwd(rawCwd)
  ...
  createParallelTask(config: { agentId: string, mode: string, flags: string[], prompt: string, worktreeCount: number, model?: string, reasoning?: string, verbosity?: string }): { sessionIds: string[], groupId: string } {
    ...
    for (let i = 0; i < count; i++) {
      const rawCwd = supportsWorktree && usedWorktrees[i]
        ? usedWorktrees[i].path
        : (this.workspace?.repository?.path || process.env.HOME)
      const cwd = resolveValidCwd(rawCwd)
  ...
  startAgentWithConfig(sessionId: string, config: any) {
    ...
    this.outputFilter.startCommand(sessionId, command)
