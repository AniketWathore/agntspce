...
export class OutputFilterService {
  ...
  // Start tracking a command that was programmatically written to the PTY.
  // This replaces the need for the wrapper's "agntspce $ <command>" marker line
  // (removed to eliminate double-echo). Called from sessionManager when a command
  // is written via startAgentWithConfig.
  startCommand(sessionId: string, commandStr: string): void {
    const detected = this._detectCommand(commandStr)
    if (!detected) return
    this.clearTimer(sessionId)
    this.finalizeCommand(sessionId)
    this.commandBuffers.set(sessionId, {
