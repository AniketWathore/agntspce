...
// Electron 34+ on Windows has known GPU compositing issues that cause
// white screens. Disable GPU acceleration for a reliable rendering path.
...
function createWindow() {
  // Ensure preload script exists before creating window
  const preloadPath = path.join(app.getAppPath(), 'dist-electron/preload.js')
  const preloadSrc = path.join(app.getAppPath(), 'electron/preload.cjs')
  if (!existsSync(preloadPath) && existsSync(preloadSrc)) {
    mkdirSync(path.join(app.getAppPath(), 'dist-electron'), { recursive: true })
    copyFileSync(preloadSrc, preloadPath)
    console.log('[main] Copied preload.cjs → dist-electron/preload.js')
  }
...
app.whenReady().then(async () => {
  ...
  // Start the Express + Socket.IO server BEFORE creating the window,
  // so the renderer's Socket.IO client can connect immediately.
  await startServer()
