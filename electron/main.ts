    ...
    : [
        ...
        },
        {
          ...
          submenu: [
            ...
            { role: 'redo' as const },
            { type: 'separator' as const },
            // Route clipboard ops through renderer so they work with both
            // xterm.js terminals (canvas-based, need term.getSelection())
            // and DOM elements (input/textarea/contentEditable).
            // Keyboard shortcuts (Ctrl+C/V/A/X) are handled by xterm's
            // attachCustomKeyEventHandler — no accelerators to avoid conflict.
            { label: 'Cut', click: () => sendMenuAction('edit-cut') },
            { label: 'Copy', click: () => sendMenuAction('edit-copy') },
            { label: 'Paste', click: () => sendMenuAction('edit-paste') },
            { label: 'Select All', click: () => sendMenuAction('edit-select-all') },
            { type: 'separator' as const },
            { role: 'find' as const },
          ],
        },
...
function createWindow() {
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.error('[window] Renderer process is unresponsive')
  })
