...
from agntspce_search.activation import enforce as enforce_activation
from agntspce_search.cache import find_index_from_cache_folder, resolve_cache_folder
from agntspce_search.index import SembleIndex
from agntspce_search.index.types import PersistencePath
from agntspce_search.stats import format_savings_report
from agntspce_search.types import ContentType
from agntspce_search.utils import format_results, is_git_url, resolve_chunk
...
def main() -> None:
    """Entry point for the agntspce-search command-line tool."""
...
def _mcp_main() -> None:
    parser = argparse.ArgumentParser(
        prog="agntspce-search",
        description="Instant local code search for agents.",
    )
    _add_content_args(parser)
    args = parser.parse_args()
    if any(find_spec(dep) is None for dep in get_package_extras("agntspce_search", "mcp")):
        print("MCP dependencies are not installed. Run: pip install 'agntspce_search[mcp]'", file=sys.stderr)
        raise SystemExit(1)
...
def _cli_main() -> None:
    parser = argparse.ArgumentParser(prog="agntspce-search")
    ...
    sub.add_parser("install", help="Interactively configure agntspce-search across coding agents.")
    sub.add_parser("uninstall", help="Interactively remove agntspce-search configuration from coding agents.")
    ...
    if args.command == "savings":
        ...
        from agntspce_search.installer import run
