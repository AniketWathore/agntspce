...
from agntspce_search.types import Chunk
from agntspce_search.utils import resolve_model_name
...
def _load_cached(model_path: str) -> StaticModel:
    ...
    try:
        ...
        if "connection" in err_msg or "connect" in err_msg or "resolve" in err_msg:
            raise OfflineModelError(
                f"Embedding model ({model_path}) download requires internet access.\n"
                "This is expected on first run — the ~66MB model is downloaded once from HuggingFace.\n"
                "Run `agntspce-search search` with a working internet connection to trigger the download,\n"
                "or set AGNTSPCE_SEARCH_MODEL_NAME to a local model path."
            ) from exc
        raise
    finally:
        disable_progress_bars()
