...
def _load_ignore_for_dir(directory: Path) -> GitIgnoreSpec | None:
    """Loads a gitignore and agntspce_searchignore for a dir."""
    gitignore = directory / ".gitignore"
    agntspce_searchignore = directory / ".agntspce_searchignore"

    lines = []
    if gitignore.is_file():
        lines.extend(gitignore.read_text(encoding="utf-8", errors="ignore").splitlines())
    if agntspce_searchignore.is_file():
        lines.extend(agntspce_searchignore.read_text(encoding="utf-8", errors="ignore").splitlines())
    if lines:
