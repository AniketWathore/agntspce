...
from agntspce_search.installer.agents import SEMBLE_END, SEMBLE_START, Action

JsonObjectResult = tuple[Node, bytes] | Literal["skipped", "error"]
_T = TypeVar("_T")

_CODEX_MCP_HEADER = "[mcp_servers.agntspce_search]"
_CODEX_MCP_BLOCK = '[mcp_servers.agntspce_search]\ncommand = "uvx"\nargs = ["--from", "agntspce_search[mcp]", "agntspce_search"]\n'
...
def replace_or_append_marked(path: Path, content: str) -> Action:
    """Replace the marked agntspce_search section in path, or append it if absent."""
    path.parent.mkdir(parents=True, exist_ok=True)
    existed = path.exists()
...
def remove_marked(path: Path) -> Action:
    """Remove the marked agntspce_search section from path."""
    if not path.exists():
        return "not-found"
...
def _strip_toml_section(text: str, header: str) -> str:
    ...
    prefix = header.strip()[1:-1]  # "[mcp_servers.agntspce_search]" → "mcp_servers.agntspce_search"
...
def merge_toml_block(path: Path) -> Action:
    """Add (or refresh) the agntspce_search [mcp_servers.agntspce_search] table in a Codex config.toml as text."""
    path.parent.mkdir(parents=True, exist_ok=True)
    existed = path.exists()
    existing = path.read_text(encoding="utf-8") if existed else ""
...
def remove_toml_block(path: Path) -> Action:
    """Remove the agntspce_search [mcp_servers.agntspce_search] table from a Codex config.toml, leaving the rest."""
    if not path.exists():
        return "not-found"
    existing = path.read_text(encoding="utf-8")
