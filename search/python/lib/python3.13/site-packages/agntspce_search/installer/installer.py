...
from agntspce_search.installer.agents import (
    AGENTS,
    INSTRUCTIONS,
    AgentTarget,
    Mode,
...
from agntspce_search.installer.config import (
    merge_json_member,
    merge_toml_block,
    remove_json_member,
    remove_marked,
...
def merge_mcp(agent: AgentTarget) -> WriteResult:
    """Add the agntspce_search MCP entry to the agent's config."""
    assert agent.mcp is not None
    path = agent.mcp.path
    return WriteResult(path, merge_json_member(path, agent.mcp.key, "agntspce_search", agent.mcp.entry))


def remove_mcp(agent: AgentTarget) -> WriteResult:
    """Remove the agntspce_search MCP entry from the agent's config."""
    assert agent.mcp is not None
    path = agent.mcp.path
    return WriteResult(path, remove_json_member(path, agent.mcp.key, "agntspce_search"))
...
def _apply_subagent(agent: AgentTarget, mode: Mode) -> WriteResult | None:
    ...
    try:
        src = files("agntspce_search").joinpath(f"agents/{agent.id}{dest.suffix}").read_text(encoding="utf-8")
...
_INTEGRATIONS: list[_Integration] = [
    _Integration(
        "mcp", "MCP server", "lets the agent call agntspce_search directly as a tool", _apply_mcp, AgentTarget.resolved_mcp_path
    ),
    _Integration(
        "instructions",
        "Instructions",
    ...
    _Integration(
        "subagent",
        "Sub-agent",
        "installs a dedicated agntspce_search-search sub-agent",
        _apply_subagent,
...
def run(mode: Mode) -> None:
    """Interactively install or uninstall agntspce_search across coding agents."""
    ...
    question = "Proceed?" if install else "Remove agntspce_search configuration?"
