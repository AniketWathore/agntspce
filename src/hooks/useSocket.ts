...
import type { WorkspaceInfo, SessionState, TerminalOutput, StatusChange, BranchChange, WorkspaceChange, AgentConfig, AgentStartConfig, FilterEvent, FilterStats, CommandEvent, ExecutionEvent, RtkStats } from '../types'
...
export function useSocket(): UseSocketReturn {
  ...
  const [rtkStats, setRtkStats] = useState<RtkStats>({ totalOriginalBytes: 0, totalFilteredBytes: 0, totalOriginalTokens: 0, totalFilteredTokens: 0, eventsProcessed: 0, commandsProcessed: 0 })
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      setFilterHistory([])
      setCommandHistory([])
      setRtkStats({ totalOriginalBytes: 0, totalFilteredBytes: 0, totalOriginalTokens: 0, totalFilteredTokens: 0, eventsProcessed: 0, commandsProcessed: 0 })
      setExecutionHistory([])
    })
  ...
  socket.on('filter-stats', (data: { stats: RtkStats }) => {
    setRtkStats(data.stats)
  })
  ...
  const chatGetModels = useCallback((): Promise<any[]> => {
    return new Promise((resolve) => {
      ...
      setTimeout(() => { socketRef.current?.off('chat-models', handler); resolve([]) }, 5000)
    })
  }, [])
  ...
  const chatSend = useCallback((threadId: string, providerId: string, content: string): Promise<any> => {
    return new Promise((resolve) => {
      ...
      setTimeout(() => { socketRef.current?.off('chat-response', handler); resolve(null) }, 30000)
    })
  }, [])
  ...
  const chatGetHistory = useCallback((threadId: string): Promise<any> => {
    return new Promise((resolve) => {
      ...
      setTimeout(() => { socketRef.current?.off('chat-history', handler); resolve({ messages: [] }) }, 5000)
    })
  }, [])
