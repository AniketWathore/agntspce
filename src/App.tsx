    ...
    const unsub = window.electronAPI?.onMenuAction?.((action, data) => {
      switch (action) {
        ...
        case 'edit-select-all': {
          const editAction = action === 'edit-cut' ? 'cut' : action === 'edit-copy' ? 'copy' : action === 'edit-paste' ? 'paste' : 'selectAll'
          // Dispatch custom event for terminal components (xterm.js canvas
          // rendering — DOM execCommand doesn't work with canvas selection).
          document.dispatchEvent(new CustomEvent('app-edit-' + editAction))
          // Fall back to execCommand for DOM input/textarea/contentEditable.
